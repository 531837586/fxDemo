...
- (void)fx_setValue:(nullable id)value forKey:(NSString *)key{
    ...
    if (![self.class accessInstanceVariablesDirectly]) {
        @throw [NSException exceptionWithName:@"FXUnknownKeyException" reason:[NSString stringWithFormat:@"****[%@valueForUndefinedKey:this class is not key value coding-compliant for the key name.****]", self] userInfo:nil  ];
    }
    ...
    // 3. 判断是否能直接赋值实例变量
    if (![self.class accessInstanceVariablesDirectly]) {
        @throw [NSException exceptionWithName:@"FXUnknownKeyException" reason:[NSString stringWithFormat:@"****[%@ %@]: this class is not key value coding-compliant for the key name.****]", self, NSStringFromSelector(_cmd)] userInfo:nil];
    }
}
...
- (nullable id)fx_valueForKey:(NSString *)key {
    ...
    //2. 找到相关方法 get<Key> <key> countOf<Key> objectInM<Key>AtIndex
    //key 要大写
    NSString *Key = key.capitalizedString;
    // 拼接方法
